...
class ScannerCog(commands.Cog, name="Scanner"):
    ...
    async def scan_loop(self) -> None:
        try:
            ...
            log.exception("Erreur scan_loop: %s", exc)

    @scan_loop.before_loop
    async def before_scan(self) -> None:
        await self.bot.wait_until_ready()

    # ─── Boucle live 10s — update embed ──────────────────────────────────────
    ...
    async def live_loop(self) -> None:
        try:
            ...
            log.exception("Erreur live_loop: %s", exc)

    @live_loop.before_loop
    async def before_live(self) -> None:
        await self.bot.wait_until_ready()
    ...
    async def _tick_scan(self) -> None:
        ...
        online_now = await ng_api.fetch_online_players(self._session)
        ...
        active = [p for p in all_players if p["online"] and not p["out_until"]]
    ...
    async def _tick_live(self) -> None:
        all_players = await db.get_all_players()
        active = [p for p in all_players if p["online"] and not p["out_until"]]
        await self._update_live(all_players, len(active))

    # ─── Live embed ───────────────────────────────────────────────────────────

    async def _update_live(self, players: list[dict], nb_active: int) -> None:
        ...
        emb = embeds.embed_live(players, nb_active)
